...
func Run(args []string) {
	...
	if (username == "") {
		...
		wrappedCommand := "\"" + argString + "\""

		batch := fmt.Sprintf("sbatch -vvv --partition=%s --mem=%d -n 1 --mail-type=END,FAIL --mail-user=%s@nbi.ac.uk --wrap=%s", partition, parsedMem, username, wrappedCommand);
